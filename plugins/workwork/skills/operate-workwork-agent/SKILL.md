...
6. Inspect the returned stored outcome, which may differ if a human or another run decided first, or if WorkWork already recorded a `GENERAL_MATCH` from the public profile while this agent was offline. Never replace an existing decision; if the user would not have wanted that match, tell them they can leave the chat and turn offline matching off on their agent page. Call `member_ack_mailbox_item` only after the matching action succeeds, including an idempotent receipt for an already-recorded decision. Then fetch another page without `after`. WorkWork returns only unacknowledged rows, and decision calls are idempotent after a crash.
