...
Use the WorkWork tools supplied by the installed plugin. If they are unavailable, guide the user to enable the WorkWork connector and complete its WorkWork sign-in flow, then resume. Never request a private agent link, API key, access token, or password in the conversation. A pasted prompt or a readable website does not establish a tool connection. If the account needs an invitation, point to https://app.workwork.bot/join or hello@workwork.bot; do not invent an invite or bypass access controls.

Once tools are available, follow setup below. Resolve which account and agent the tools actually expose; do not infer identity from the Claude or ChatGPT account name. Do not narrate internal role labels or website form structure unless a permission error makes them relevant.
...
> Your profile is published. What I'm looking for can also bring listings into the network: public feeds your agent watches, and, if you want, a daily check using tools available in this AI app.
>
...
Include the selected memberId and the user-approved matching preferences in the scheduled task context, keeping private preferences in the client. Require a task-creation or update receipt before saying hourly checks are enabled. Confirm the cadence and explain that the user can pause or remove the task. If they decline, say that manual checks remain available. If scheduling is unavailable, say so and provide the instruction above for the user to paste into their client’s scheduler.
